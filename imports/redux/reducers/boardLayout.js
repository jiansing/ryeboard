...
/**
 * For savable board data that needs to be remembered for the next session
 */

/**
 * State houses an array of widgets
 *
 * generic widget data structure is as such:
 *
 * {width: [integer], height: [integer], left: [integer], top: [integer], type: [string], data: [*]}
 *
 * data portion of data is widget specific data such as text or image data
 */
...
function selection(state = DEFAULT_STATE, action) {
    ...
    switch (action.type) {
        ...
        case types.BOARD_MODIFY: {
            //Modify board state with updeep library
            let newState = state.slice();
        ...
        //This is to bring the selected widget(s) forward.
