/**
 * Constants for different types of actions
 */

//Board Creator Action Constants
export const BOARD_ADD= "BOARD_ADD";
export const BOARD_REMOVE = "BOARD_REMOVE";
export const BOARD_MODIFY = "BOARD_MODIFY";
export const BOARD_SET_SELECTED_WIDGET_DATA= "BOARD_SET_SELECTED_WIDGET_DATA";
export const BOARD_SELECT_WIDGET = "BOARD_SELECT_WIDGET";
export const BOARD_MULTI_SELECT_WIDGET = "BOARD_MULTI_SELECT_WIDGET";
export const BOARD_DESELECT_WIDGET = "BOARD_DESELECT_WIDGET";
export const BOARD_DESELECT_ALL_WIDGET = "BOARD_DESELECT_ALL_WIDGET";
export const BOARD_DRAG_WIDGET = "BOARD_DRAG_WIDGET";
export const BOARD_MULTI_DRAG_WIDGET = "BOARD_MULTI_DRAG_WIDGET";

//Settings Creator Action Constants
export const SETTINGS_MODIFY = "SETTINGS_MODIFY";
export const SETTINGS_ADD = "SETTINGS_ADD";
export const SETTINGS_REMOVE = "SETTINGS_REMOVE";

export const SET_MUTABLE = "SET_MUTABLE";
export const SET_STATE = "SET_STATE";